...
func (p *Aggregator) Start() {
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		default:
			p.middleware.ConsumeAndProcess(game_positive_reviews, p.aggregateGames)
			p.sendResults()
		}
...
func (p *Aggregator) shouldKeep(game prot.Game, top int) (bool, error) {
	if len(p.games) < top {
		return true, nil
...
func (p *Aggregator) saveGame(game prot.Game, top int) error {
	if len(p.games) < top {
...
func (p *Aggregator) aggregateGames(msg []byte, finished *bool) error {
	...
	for idx, _ := range p.config.Top {
		game, err, _ := protocol.DeserializeGame(msg[idx:])
		if err != nil {
