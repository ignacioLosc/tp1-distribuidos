...
func CreateMiddleware() (*Middleware, error) {
	conn, err := connectRabbitMQ(5)
	if err != nil {
		log.Fatalf("action: [connection to rabbitmq] | msg: Could not establish connection: %v", err)
	}
	return &Middleware{conn, make(map[string]*amqp.Channel), make(map[string]amqp.Queue)}, nil
}
...
func (m *Middleware) ConsumeAndProcess(channelName string, queueName string, processFunction func([]byte, *bool) error) {
	ch, ok := m.channels[channelName]
	if !ok {
		return
	}
...
func (m *Middleware) ConsumeExchange(channelName string, queueName string, processFunction func([]byte, string, *bool) error) {
	ch, ok := m.channels[channelName]
	if !ok {
		return
	}
...
func (m *Middleware) ConsumeExchange2(channelName string, queueName string, msgChan chan MsgResponse) {
	ch, ok := m.channels[channelName]
	if !ok {
		return
	}
